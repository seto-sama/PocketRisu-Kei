...
PocketRisu 会在手机上本地构建应用,因此构建时间取决于手机性能。
...
> Termux 维护者已于 2020 年停止更新 Play Store 版本,无法再安装 PocketRisu 所需的最新软件包(Node.js 24.15+)。
...
4. `pnpm install` — JavaScript 依赖和可选的原生加速模块
5. `pnpm build` — 前端打包
