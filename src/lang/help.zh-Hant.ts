/**
 * PocketRisu Kei NodeOnly — Traditional Chinese help texts (`language.help`).
 *
...
export const helpZhHant = {
        ...
        "systemPrompt": "此提示詞已不再使用，也不會包含在請求中。",
        ...
        "gptVisionQuality": "設定圖片輸入使用的細節層級或媒體解析度。品質越高，越能辨識細節，但會使用更多輸入 Token，並可能增加延遲。",
        "genTimes": "主回應生成的總次數。首次回應後會依序自動執行一般重新生成操作。任一生成呼叫工具時會停止剩餘的自動重新生成，且自動重新生成不會播放 TTS。數值越高，請求次數與費用越高。",
        ...
        "antiClaudeOverload": "當 Claude 發生過載時，PocketRisu Kei 會嘗試繼續發送相同的提示詞以防止請求中斷，藉此減少過載機率。此功能僅適用於串流回應 (Streamed Responses)，對非官方 API 端點可能無效。",
        ...
        "defaultVariables": "可在此處設定自訂預設變數。使用 `<variable name>=<variable value>` 格式，每行一個。例如：`name=PocketRisu Kei`，可在觸發式和 CBS 變數中使用，如：`{{getvar::A}}`、`{{setvar::A::B}}` 或 `{{? $A + 1}}`。若提示詞模板的預設變數與角色的預設變數名稱相同，系統將使用角色的預設變數。",
        ...
        "automaticCachePoint": "如果沒有明確設定快取點，則自動為最近最多 3 則使用者訊息指定快取點。",
        ...
        "inlayImageLossy": "使用有損 WebP 壓縮；關閉後會儲存為無損 WebP。",
        "inlayImageQuality": "設定有損 WebP 的品質；數值越高，檔案越大。",
        ...
        "disableToggleBinding": "顯示將切換值綁定至個別對話的功能。",
        "keepSessionAlive": "透過播放靜音音訊讓分頁保持活躍，防止瀏覽器工作階段因閒置而過期。可能需要重新整理後才能生效。",
