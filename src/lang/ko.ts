...
export const languageKorean = {
  ...
  deleteRerollMessage: "재생성된 메시지 삭제",
  deleteRerollMessageConfirm: "현재 선택된 재생성 메시지를 삭제할까요?",
  rerollConfirm: "재생성하여 신규 메시지를 목록에 추가 할까요?",
  noSwipesRerollConfirm:
    "현재는 추가 재생성된 메시지가 없습니다. 재생성하여 신규 메시지를 목록에 추가 할까요?",
  chatList: "채팅 리스트",
  ...
  imageSettings: {
    ...
    vibeFile: "바이브 파일",
    invalidVibeFile: "올바르지 않은 바이브 파일입니다.",
  ...
  accessibility: "접근성",
  sendWithEnter:
    "엔터키로 메시지 보내기(체크 해제시 Shift + Enter가 메시지 전송으로 변경.)",
  ...
  enableBlockPartialEdit:
    "블록 부분 수정 활성화",
  enableDragPartialEdit: "드래그 부분 수정 활성화",
  setNodePassword: "보안을 위해 비밀번호를 정해주세요",
  inputNodePassword:
    "비밀번호를 입력해주세요. 비밀번호를 잊어버렸다면 save/__password를 지우고 서버를 재시작해주세요.",
  simple: "간단",
  advanced: "고급",
  active: "활성화",
  hub: "RisuRealm",
  ...
  hideMessagePageCount: "메시지 페이지 숫자 숨기기",
  popularityLevel: "인기도 {}",
  colorScheme: "색상 팔레트",
  ...
  pluginModelProfileDescription: "활성화된 플러그인 API 3.0 프로바이더가 등록한 모델입니다. 이 프리셋을 사용하려면 해당 플러그인을 계속 활성화해야 합니다.",
  ...
  pluginVersionWarn:
    "이 플러그인은 {{plugin_version}} 버전으로, PocketRisu Kei와 호환되지 않습니다. 플러그인을 {{required_version}} 버전으로 업데이트하세요.",
  ...
  modelPresetStartWithUser: "유저 턴으로 시작",
  ...
  mcpAccessPrompt: (tool: string, action: string) =>
    `${appendKoreanJosa(tool, "이/가")} ${appendKoreanJosa(`"${action}"`, "을/를")} 시도하고 있습니다. 허용하시겠습니까?`,
  ...
  pluginRiskDetectedAlert:
    "API 키나 개인 데이터를 훔칠 수도 있는 위험한 코드가 감지되었습니다. 이 플러그인을 설치하지 않는 것을 권장합니다.",
  doNotInstall: "설치하지 않음",
  continueAnyway: "무시하고 계속",
  pluginNoRiskButAlert:
    "위험한 코드는 감지되지 않았지만, 신뢰할 수 없는 출처의 플러그인을 설치하면 보안 위험이 발생할 수 있습니다.",
  ...
  disableFirstMessageConfirm: "첫 메시지를 비활성화할까요? 컨텍스트에서 제외됩니다.",
  disableAbove: "기억 한계선 긋기",
  disableAboveHelp: "해당 선 위의 모든 메시지를 컨텍스트에서 제거합니다.",
  bookmark: "북마크",
  branch: "분기 생성",
  fetchLogConsent: (pluginName: string) =>
    `플러그인 ${appendKoreanJosa(pluginName, "이/가")} 로그를 가져오려고 합니다. 민감한 정보가 노출될 수 있습니다. 허용하시겠습니까?`,
  getFullDatabaseConsent: (pluginName: string) =>
    `플러그인 ${appendKoreanJosa(pluginName, "이/가")} 전체 데이터베이스에 접근하려고 합니다. 민감한 정보가 노출될 수 있습니다. 허용하시겠습니까?`,
  mainDomAccessConsent: (pluginName: string) =>
    `플러그인 ${appendKoreanJosa(pluginName, "이/가")} 메인 Document에 접근하려고 합니다. 민감한 정보가 노출될 수 있습니다. 허용하시겠습니까?`,
  replacerPermissionConsent: (pluginName: string) =>
    `플러그인 ${appendKoreanJosa(pluginName, "이/가")} 채팅 내용을 교체할 권한을 요청합니다. 대화 조작에 사용될 수 있습니다. 허용하시겠습니까?`,
  providerPermissionConsent: (pluginName: string) =>
    `플러그인 ${appendKoreanJosa(pluginName, "이/가")} 프로바이더 접근 권한을 요청합니다. 무단 API 호출이 발생할 수 있습니다. 허용하시겠습니까?`,
  sendChatConsent: (pluginName: string) =>
    `플러그인 ${appendKoreanJosa(pluginName, "이/가")} 사용자 대신 채팅 메시지를 전송할 권한을 요청합니다. AI 응답이 트리거됩니다. 허용하시겠습니까?`,
  inlayPermissionConsent: (pluginName: string) =>
    `플러그인 ${appendKoreanJosa(pluginName, "이/가")} 인레이 에셋 접근 권한을 요청합니다. 채팅에 첨부된 파일이 노출될 수 있습니다. 허용하시겠습니까?`,
  resetPluginPermission: "권한 설정 초기화",
  resetPluginPermissionConfirm:
    '플러그인 "{}"에 저장된 권한 허가 기록을 모두 삭제하시겠습니까?\n\n다음에 플러그인이 권한을 요청할 때 다시 묻게 됩니다.',
  resetPluginPermissionDone: '"{}"의 권한 허가 기록을 초기화했습니다',
  ...
  remoteAccess: "원격 접속",
  remoteAccessDesc:
    "원격 접속 링크를 사용하여 다른 기기에서 이 서버에 접속할 수 있습니다.",
  ...
  systemLogs: "시스템 로그",
  systemLogsDesc:
    "서버 및 클라이언트에서 발생한 에러, 경고, 정보 이벤트를 확인합니다.",
  ...
  realmViewSuffix: "보기",
  relatedGithubDesc: "Star를 눌러 원본 포켓리스 프로젝트를 응원해주세요.",
