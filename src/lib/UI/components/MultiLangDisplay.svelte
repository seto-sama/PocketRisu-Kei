...
<div class={cn('flex flex-col', className)}>
        ...
        <div class="flex max-w-fit flex-wrap items-center gap-2 px-1 pb-1">
            {#if sortedLangs.priority}
                {#if sortedLangs.priority !== 'xx' || Object.keys(valueObject).length === 1}
                    <Button size="sm" variant={selectedLang === sortedLangs.priority ? 'primary' : 'outline'} className={selectedLang === sortedLangs.priority ? '' : 'text-subtext'} aria-pressed={selectedLang === sortedLangs.priority} onclick={(e) => {
                        e.stopPropagation()
            ...
            {#each sortedLangs.rest as lang}
                {#if lang !== 'xx' || Object.keys(valueObject).length === 1}
                    <Button size="sm" variant={selectedLang === lang ? 'primary' : 'outline'} className={selectedLang === lang ? '' : 'text-subtext'} aria-pressed={selectedLang === lang} onclick={(e) => {
                        e.stopPropagation()
                        selectedLang = lang
    ...
    {/if}
    {#if markdown}
        <div class={cn('ml-2 max-w-full wrap-break-word text chat chattext prose', contentClass)}>
