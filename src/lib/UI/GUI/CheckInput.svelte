...
<label 
    class={"flex items-center gap-2 " + (disabled ? "cursor-not-allowed opacity-50" : "cursor-pointer") + (className ? " " + className : "") + (grayText ? " text-subtext" : " text-maintext")}
    class:mr-2={margin}
...
>
    ...
    <span 
        class={"w-5 h-5 min-w-5 min-h-5 flex justify-center items-center transition-colors duration-200 "
            + (card
                ? `rounded border ${check ? (checkedColor === 'primary' ? 'border-primary bg-primary' : 'border-lightborderc bg-lightborderc') : `border-darkborderc ${cardUncheckedFill ? 'bg-darkbg/50 mix-blend-multiply' : 'bg-transparent'}`}`
    ...
    >
        {#if check}
            <CheckIcon class="w-3 h-3 text-themewhite" strokeWidth={5} aria-hidden="true" />
        {/if}
    </span>
