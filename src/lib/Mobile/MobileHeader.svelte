...
<IconButtonGroup size="lg" className="w-full px-4 h-16 border-b border-b-darkborderc bg-darkbg justify-start gap-2">
    ...
    {:else if $MobileGUIStack === 1}
        <div class="flex items-stretch w-2xl max-w-full">
            <input placeholder={language.search + '...'} bind:value={$MobileSearch} class="risu-field-border peer outline-hidden text-maintext px-2.5 py-2 min-w-0 bg-transparent rounded-md input-text text-xl grow mx-4 resize-none overflow-y-hidden overflow-x-hidden max-w-full">
        </div>
    {:else}
        <span class="font-bold text-lg">{PRODUCT_NAME}</span>
